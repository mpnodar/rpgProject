...
{
	if (!gm->questHelpBeggar->FoughtShadyPeople) {
	...
	else if (gm->questHelpBeggar->FoughtShadyPeople && !gm->questHelpBeggar->RecievedMoney) {
		dialogueTree[0] = new DialogueNode("All right stop! I yield! Here is the money!\n",
			"1. Accept Gold\n",
			{ GaveGold });
	}
	else {
		dialogueTree[0] = new DialogueNode("We gave you the gold now leave us alone!\n",
			"1. Leave\n",
			{ Ended });
	}
}
